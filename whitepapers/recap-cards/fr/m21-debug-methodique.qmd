...
Un agent unique convient pour les bugs localisés sur 1 à 3 fichiers. Pour les problèmes qui traversent plusieurs composants interconnectés (un bug de réseau qui affecte l'auth qui affecte la base de données), déléguer l'exploration à un sous-agent via l'Agent tool (anciennement Task) préserve le contexte principal.
...
Les erreurs du sous-agent ne polluent pas le contexte de l'agent principal.
