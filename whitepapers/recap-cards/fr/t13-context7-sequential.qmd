...
**Triggers** : demande explicite ("raisonne étape par étape", "utilise Sequential Thinking"), ou problèmes impliquant 3+ composants interconnectés. Il n'existe pas de flag CLI dédié à Sequential Thinking : `--think-hard` et `--ultrathink` sont d'anciens mots-clés pour l'extended thinking natif du modèle, aujourd'hui purement cosmétiques et sans effet (la profondeur de raisonnement se règle via le curseur `/model` ou `effortLevel`).
