...
Chaque agent dispose de sa propre fenêtre de contexte (1M tokens avec Opus 4.8) et peut recevoir une whitelist d'outils distincte. Limiter un agent aux outils `Read`, `Glob`, `Grep` empêche toute modification accidentelle pendant la phase d'analyse.
...
Prérequis : Claude Code v2.1.32+, modèle Opus 4.8 recommandé, Opus 4.6+ compatible (`/model opus`), dépôt git initialisé. Navigation entre agents via `Shift+Down` en mode in-process.
...
**Reviewer dédié** : 1 agent Opus 4.8 read-only (outils `Read/Grep/Glob`) auto-déclenché sur `TaskCompleted`, pour 4 agents workers. Ratio 1:4. (Crédit : Addy Osmani)
