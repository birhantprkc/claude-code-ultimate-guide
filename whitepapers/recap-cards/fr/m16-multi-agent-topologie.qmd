...
Chaque agent dispose de sa propre fenêtre de contexte (1M tokens avec Opus 5) et peut recevoir une whitelist d'outils distincte. Limiter un agent aux outils `Read`, `Glob`, `Grep` empêche toute modification accidentelle pendant la phase d'analyse.
...
**Reviewer dédié** : 1 agent Opus 5 read-only (outils `Read/Grep/Glob`) auto-déclenché sur `TaskCompleted`, pour 3 à 4 agents workers. (Crédit : Addy Osmani)
