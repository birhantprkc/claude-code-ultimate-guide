...
## Contrôle de flux dans les pipes

En mode print, `claude -p` attend la réponse complète et l'écrit sur stdout en un seul bloc, ce qui simplifie les pipes avec des outils qui attendent une entrée complète. Le streaming temps réel est optionnel via `--output-format stream-json`.

```bash
claude -p "Génère un rapport" > rapport.md
```
