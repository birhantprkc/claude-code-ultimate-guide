...
subtitle: "Choisir entre Glob, Grep, Read, Agent et Bash selon le contexte"
card-number: "T11"
category: "Technique"
...
| `Agent` | Exploration large, multi-fichiers | Très élevé |
...
Grep est suffisant pour 90% des recherches de texte exact. Ne pas escalader vers Agent ou Bash si la pattern est connue.
...
## Agent : exploration large

L'outil Agent (anciennement nommé Task) spawne un sous-agent avec son propre contexte isolé. C'est la bonne option quand on ne sait pas où chercher, que le périmètre est vaste, ou que l'exploration nécessite plusieurs fichiers en parallèle.
...
**Principe fondamental** : Glob et Grep avant d'ouvrir des fichiers avec Read. Agent uniquement quand le périmètre est inconnu.
