...
These are the latest MRCR scores Anthropic has published; Opus 4.8, Opus 5, and Sonnet 5 scores are not yet published. Opus remains the reference model at 1M, but degradation past 256K is measurable. Sonnet collapses and is not recommended beyond 200K for precise tasks.

## Cost per Session (Approximate)

| Session type | Tokens in | Sonnet 5 | Opus 5 |
|-------------|-----------|-----------|---------|
| PR review (≤200K) | 50K | ~$0.15 | ~$0.38 |
| Refactoring (≤200K) | 150K | ~$0.50 | ~$1.25 |
| Service analysis (>200K) | 500K | ~$2.75 | ~$6.88 |
...
For direct API access only, on Opus 4.8 (previous generation). Sonnet 5 and Opus 5 ship with native 1M, no header required. Claude Code Max/Team/Enterprise plans have 1M enabled automatically. Without this header on direct API calls to Opus 4.8, requests exceeding 200K tokens return an error even on tier 4 accounts.
