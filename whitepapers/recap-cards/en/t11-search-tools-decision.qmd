...
subtitle: "Choosing between Glob, Grep, Read, Agent and Bash based on context"
card-number: "T11"
category: "Technical"
...
| `Agent` | Broad, multi-file exploration | Very high |
...
Grep is sufficient for 90% of exact text searches. Do not escalate to Agent or Bash if the pattern is known.
...
The Agent tool (formerly named Task) spawns a sub-agent with its own isolated context. It is the right option when you do not know where to look, the scope is wide, or exploration requires multiple files in parallel.
...
**Core principle**: Glob and Grep before opening files with Read. Agent only when the scope is unknown.
