...
`TeammateIdle` and `TaskCompleted` allow implementing quality gates at the agent team level: preventing a task from being marked `completed` if tests fail, for example. `ConfigChange` is for configuration auditing in enterprise environments and does not depend on Agent Teams.
