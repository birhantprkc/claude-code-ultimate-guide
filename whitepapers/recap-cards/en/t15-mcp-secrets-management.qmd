...
**`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` (v2.1.83+)**: environment variable that forces Claude Code to strip sensitive variables (`*_KEY`, `*_TOKEN`, `*_SECRET`) before passing them to subprocesses and MCP servers. Enable in production to minimize exposure surface.
