...
- **New audit prompt: permission and sandbox posture** (`tools/permissions-audit-prompt.md`): a self-contained prompt that answers one question the existing tooling does not, whether a project's permission rules still constitute a boundary. `tools/audit-prompt.md` scores the whole setup across eight dimensions and `/security-audit` scans for secrets and injection surfaces; neither detects the failure mode where a single `allow` entry resolving to an interpreter (`Bash(bash *)`, `Bash(node:*)`, `Bash(python3 *)`) silently makes every `deny` rule in every scope bypassable, since the matcher treats `bash -c '<anything>'` as a `bash` command and never as the inner one. Six phases: blanket execution grants, rule population health, deny and ask coverage, sandbox posture, scope hygiene, and permission-mode interaction. The prompt embeds the documented matcher semantics rather than letting the model infer them, because that is where audits of this kind go wrong: the stripped wrapper list (`timeout`, `nice`, `command`, bare `xargs`) versus the runners that are *not* stripped (`npx`, `docker exec`, `devbox run`), the fact that an `allow` rule stops at an unknown env-var assignment while `deny` and `ask` match past any, that `:*` is only recognized at the end of a pattern, that protected paths are not pre-approvable by an `allow` rule in settings, that `sandbox.allowUnsandboxedCommands: false` constrains only the `dangerouslyDisableSandbox` retry and not `excludedCommands`, and that in auto mode `allow`, `ask`, and `deny` resolve *before* the classifier, so a narrow rule such as `Bash(git push:*)` does not merely skip a prompt, it removes classifier review from that action. Scored out of 100 with a deliberately lopsided weighting: any arbitrary execution grant zeroes 30 points on its own. Includes a fleet triage sweep that ranks repositories by exposure before running the full pass, so a thirty-repo park is not audited one indistinguishable finding at a time. Every finding must carry a reproduction command; the prompt explicitly forbids padding and forbids recommending `bypassPermissions` as a remedy for prompt fatigue.

### Documentation

- **81 stale Claude model-version references found and fixed across 32 files** (`guide/ultimate-guide.md`, `guide/cheatsheet.md`, `guide/core/*.md`, `guide/diagrams/09-cost-and-optimization.md`, `guide/ecosystem/*.md`, `guide/workflows/*.md`, `guide/ops/api-gateway.md`, `guide/roles/agent-evaluation.md`, `guide/learning-path/03-memory.md`, `machine-readable/reference.yaml`, `machine-readable/cowork-reference.yaml`, `examples/**`, `docs/workflows/whitepaper-build.md`, `scripts/translate-guide.py`): a user screenshot of the Evolution Timeline table stopping at Opus 4.7 triggered a full sweep. 10 parallel agents audited every living file (dated snapshots in `docs/resource-evaluations/` were excluded on purpose), then 6 parallel agents fixed what they found, all pricing verified live against `platform.claude.com/docs/en/about-claude/models/overview` rather than assumed: Claude Opus 5 at $5/$25 per Mtok standard ($10/$50 fast mode), Claude Sonnet 5 at $3/$15 standard ($2/$10 promotional through 2026-08-31), Claude Haiku 4.5 at $1/$5 (was still showing a stale $0.80/$4.00 in two places). The most consequential fix wasn't prose: `examples/hooks/bash/session-summary.sh`'s pricing fallback function had no case entry for Sonnet 5, Opus 5, or Opus 4.8 and silently returned $0 for anyone running current default models. Three GitHub Actions review templates (`claude-code-review.yml`, `claude-code-review-batched.yml`, `claude-pr-auto-review.yml`) and two scripts (`sonnetplan.sh`, `test-prompt-caching.ts`) had old model IDs hardcoded into code people actually run, not just documentation. `guide/workflows/agent-teams.md` repeated "Opus 4.6 required" across 9 separate lines, one root cause fixed in one pass. `machine-readable/cowork-reference.yaml` still names Opus 4.6 as Cowork's model in 6 places; Cowork's actual current model isn't confirmed anywhere in this repo or on Anthropic's public pages (checked), so those lines were hedged with a verify-before-quoting note rather than guessing a newer version. `docs/drafts/README-new.md` was left untouched, it carries a Feb 12, 2026 snapshot badge and reads as an intentionally frozen draft.
