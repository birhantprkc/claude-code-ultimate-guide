...
Current model defaults make the missing model mix material. Claude Code defaults to Opus 5.5 across Pro, Max, Team, and Enterprise on the direct service; Foundry retains Sonnet 4.5. Organization settings can override the default. A historical or mixed-model average is not a forecast for an Opus-heavy deployment. Source: [Claude Code model configuration](https://code.claude.com/docs/en/model-config), verified 2026-09-24.
...
At current public API rates, Opus 5.5 costs $4 per million input tokens and $20 per million output tokens, Sonnet 5 costs $2 and $10, and Haiku 4.5 costs $1 and $5. The twofold Opus-to-Sonnet rate difference is meaningful only if both models clear the same quality gate. Source: [Claude model pricing](https://platform.claude.com/docs/en/about-claude/pricing), verified 2026-09-24.
...
A gateway contract does not approve every routed provider. Back Market reported that its organization allowed only a small internally reviewed subset of the roughly 70 providers then available through OpenRouter, and that routing to ten providers means ten data policies to verify. The auto-generated transcript garbles the size of that shortlist, so the exact count is not established, and the speaker explicitly did not present the statement as legal advice. Keep a provider allowlist and review each provider's contract, processing path, and controls before enabling it. See [Model Allowlists](./api-gateway.md#4-model-allowlists) for the named providers and the liability boundary. Source: [Nicolas Martignole at 38:17](https://www.youtube.com/watch?v=DRtd8S_3E-w&t=2297s), published 2026-07-22.

Moving traffic to an aggregator can move the bill, but the mechanism is model substitution rather than a better rate on the same model. Back Market reported going from $87,000 a month on the Anthropic API in June 2026 to $23,000 on OpenRouter in August, for the same population. The speaker attributes the drop to two changes made in the same window, not one: developers stopped using Anthropic models exclusively, with an open-weight model absorbing 53% of tokens for 35% of spend, and a per-developer budget was introduced at the same time. Because both landed together, the account does not allow either to be credited separately, and it reports no measurement of output quality or time spent. Treat it as an order of magnitude for a substitution-plus-budget move, not as a rate saving. Source: [the two-month OpenRouter review](https://www.youtube.com/watch?v=pbrP--c4alk), published 2026-09-14.
