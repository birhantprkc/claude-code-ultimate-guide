...
These figures are a starting structure, not a measured benchmark. One published account gives a comparable order of magnitude at scale: Back Market reported a flat $200 per developer per month, reset monthly, across roughly 245 developers, with an escalation path rather than a hard stop. A developer who exhausts the allowance can self-serve one $20 extension, gated by a form that asks how many sessions were run, which session cost the most, and how spend split across models. No human approves that extension; the form exists so the developer reads their own console. Beyond that, a manager submits an estimate to a review committee and the project gets its own OpenRouter workspace outside the individual allowance. The account reports the allowance holds in practice and that the extension form was well accepted, but publishes no data on task completion, quality or review time, so it calibrates the budget line and nothing else. Source: [Nicolas Martignole](https://www.youtube.com/watch?v=pbrP--c4alk), published 2026-09-14.

A model allowlist is not a provider allowlist. An aggregator can expose the same model through several infrastructure providers, each with a different contract and processing path. Back Market reported allowing only a small internally reviewed subset of the roughly 70 providers then available through OpenRouter, and named Google Vertex, Fireworks and Amazon Bedrock among them. The auto-generated transcript garbles the size of that shortlist, so the exact count is not established, and the speaker did not present the statement as legal advice. A later account from the same speaker describes working with about ten providers and adds Microsoft Azure and Mistral to the named set; that figure covers providers actually used, which is not necessarily the same list as the approved one. Review and approve each provider separately. Sources: [Nicolas Martignole at 38:17](https://www.youtube.com/watch?v=DRtd8S_3E-w&t=2297s), published 2026-07-22, and [the two-month OpenRouter review](https://www.youtube.com/watch?v=pbrP--c4alk), published 2026-09-14.

The same account states the rule the count only illustrates: routing to ten providers means ten data policies to verify. Responsibility does not transfer to the aggregator. Back Market's account is explicit that OpenRouter does not carry what happens at Fireworks, Google Vertex or Amazon Bedrock on the customer's behalf, so each provider's contract, processing path and controls stay the customer's to review. A single commercial contract with the aggregator simplifies procurement, not liability.

See [Subscription Strategy](./subscription-strategy.md#6-workforce-plans-and-production-api-traffic-solve-different-problems) for the workforce and API identity split.
