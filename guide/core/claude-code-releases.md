...
- [v2.1.266](#v21266-2026-09-08), [v2.1.265](#v21265-2026-09-08), [v2.1.263](#v21263-2026-09-06): gateway fix, plugin folders and maintenance
...
- **Added**: Plugin authors can run evaluation suites with `claude plugin eval` and inspect scored JSON and HTML reports.
- **Added**: `/output-style [name]` lists or switches output styles, including through Remote Control and in cloud or other headless sessions.
- **Added**: `bashEditDiffEnabled` includes a file diff in Bash results when Bash handles edits. VS Code gains an agent map and dialogs for hooks and permission rules.
- **Fixed**: Prompt-cache reuse after an output-token cutoff or an interrupted thought. `/goal` now retries with backoff or pauses with a reason after API failures, network drops or limits.
- **Fixed** (security): Plugin extraction no longer leaves files readable by other local users, retains world-writable archive bits, or keeps stale extracted files. Bash `tee` now respects `Edit()` deny rules and destination checks; negated permission rules stay within their settings source.
- **Fixed**: One-off cloud routines could run twice after a transient error; routines using subagents could finish prematurely and mishandle retries.

### v2.1.268 (2026-09-10)

- **Fixed**: Third-party Anthropic-compatible endpoints rejecting every turn with HTTP 400 because of an Artifact input-schema regression introduced in v2.1.265.
- **Added**: WebFetch stops after 300 seconds if the response never finishes. `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` changes this deadline; 0 disables it.
- **Fixed** (security): Deny/ask rules now cover real paths behind symlinked directories. Read/Edit deny rules apply when an unanalyzable command shares the line, and respawned teammates cannot load same-named definitions from untrusted folders.
- **Fixed** (security): MCP and plugin diagnostics no longer expose credentials from git URLs or environment placeholders in MCP configuration.
- **Improved**: Bedrock, Vertex and Foundry keep tool definitions stable across the conversation. Installing, enabling or disabling plugins through `/plugin` takes effect when the menu closes.
- **Changed**: Plain `WebFetch` deny/ask rules no longer govern Artifact reads or updates. Use `Artifact` or `WebFetch(domain:claude.ai)` for that boundary.

### v2.1.267 (2026-09-09)

- **Added**: `maxEffortLevel` caps effort globally or per model under `modelSettings`, including on Bedrock, Vertex and Foundry. Users may select a lower level.
- **Added**: `--system-prompt-snapshot off` refreshes the system prompt on each request when iterating on prompt text.
- **Fixed**: Prompt-cache misses and lost thinking when tools reconnect or disappear, models change, or sessions and subagents resume. Large transcripts now retain parallel tool calls and hook output.
- **Fixed** (security): Unreadable managed `allowedHttpHookUrls`, `httpHookAllowedEnvVars` and `allowedChannelPlugins` admit nothing. Marketplace paths containing backslashes no longer bypass containment on macOS and Linux.
- **Fixed**: Cloud Cowork scheduled tasks can start when organizational settings require sandboxing; custom command, skill and subagent effort frontmatter works on models with pinned default effort.

### v2.1.266 (2026-09-08)

- **Fixed**: The v2.1.265 regression where `CLAUDE_CODE_USE_GATEWAY` alone forced Cloud-gateway sign-in and broke API-key, `apiKeyHelper` or custom-header setups. Its previous behavior is restored; no configuration change is required.

### v2.1.265 (2026-09-08)

- **Added**: `--plugin-dir` accepts a parent folder whose child folders contain plugin manifests; additions and removals are detected while running.
- **Added**: Tool results saved to disk have a 1 GB cap, with truncation indicated in the conversation preview. VS Code can archive inactive sessions, with a 14-day default.
- **Fixed**: Resumed subagents preserve prompt-cache prefixes and hook/skill placement. After a process dies during a tool call, resume retains the interrupted call and original prompt.
- **Fixed** (security): Backslashes in plugin paths no longer evade containment on macOS/Linux. Claude Code status/diff probes no longer execute nested repositories' git clean filters.
- **Fixed**: Non-interactive sessions preserve a shell `cd` across user turns. HTTP MCP connections fall back to legacy HTTP+SSE when needed.
- **Changed**: Managed `forceLoginGatewayUrl` selects gateway authentication from startup, ignoring leftover claude.ai credentials or API keys. Versions 2.1.266 and 2.1.268 fix gateway and compatible-endpoint regressions from this release.

### v2.1.263 (2026-09-06)

- **Fixed**: Bug fixes and reliability improvements. The official changelog does not identify individual changes.
...
- ⭐ **Fast mode now available for Opus 4.6**: Same model, faster output. Toggle with `/fast`. [Fast mode docs](https://code.claude.com/docs/en/fast-mode)
...
- **⚠️ Deprecation notice for npm installations**: Run `claude install` or see [getting-started docs](https://docs.anthropic.com/en/docs/claude-code/getting-started)
...
| v2.1.265 | Plugin backslash containment bypass and nested-repository git clean filters in status/diff probes |
...
*Last updated: 2026-09-12 | [Back to main guide](../ultimate-guide.md)*
