...
The pinned Liza configuration makes the boundary visible. Some high-impact planning stages require a quorum of two and mark provider diversity as `preferred`, while the coding pair uses a quorum of one. Liza's [architectural issues ledger](https://github.com/liza-mas/liza/blob/a22c12381c5d884d2586a48aaaa517bca184f9cf/specs/architecture/architectural-issues.md) records unenforced provider diversity at verdict submission and unmeasured reviewer accuracy. Manual inter-phase checkpoints apply when `auto_resume` is disabled; the pinned profile also documents autonomous progression when enabled. Human governance does not imply a human in every execution cycle. The code enforces workflow legality; it does not prove that a legal reviewer verdict is correct.
