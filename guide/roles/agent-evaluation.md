...
The judge model is typically larger than the production model (for example, using Opus 5.5 to judge outputs from Sonnet 5). It evaluates on dimensions like factual accuracy, instruction adherence, and hallucination presence. Over time the judge dataset becomes the primary signal for prompt iteration.
